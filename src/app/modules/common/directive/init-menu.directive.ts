import { Directive, AfterViewInit, Sanitizer, SecurityContext, OnInit, Input, OnChanges, SimpleChanges, Output, EventEmitter } from '@angular/core';
...
export class InitMenuDirective implements AfterViewInit, OnChanges {
  ...
  resizeWindow() {
    ...
    if (window.innerWidth < 768) {
      ...
      $('nav').removeClass('navbar-brand-center');
    }
    else {
      $('body').removeClass('fixed-navbar');
      // $('nav').removeClass('fixed-top');
      $('nav').addClass('navbar-brand-center');
    }
  }
    ...
    $.app.menu = {
      ...
      changeMenu: function (screen: string) {
        ...
        if (screen == 'sm' || screen == 'xs') {
          ...
          menuNavigation.attr('class', 'nav navbar-nav');
          // $('nav.header-navbar').removeClass('fixed-top');
          menuWrapper.attr('class', 'header-navbar navbar-expand-sm navbar navbar-horizontal navbar-fixed navbar-light navbar-without-dd-arrow navbar-shadow');

          // Change menu wrapper, menu container, menu navigation classes
          //menuWrapper.removeClass().addClass(menuWrapperClasses);

          dropdownMenu.attr('class', 'dropdown nav-item');
          dropdownMenu.find('a').attr('class', 'dropdown-toggle nav-link');
          dropdownMenu.children('ul').attr('class', 'dropdown-menu')
          dropdownMenu.children('ul').find('a').attr('class', 'dropdown-item');
          dropdownMenu.find('ul').attr('class', 'dropdown-menu');
          dropdownSubMenu.attr('class', 'dropdown dropdown-submenu');
  ...
  registerMenu() {
    ...
    $(window).on('load',  () => {
      ...
      // Toggle fullscreen
      // $('a[data-action="expand"]').off('click').on('click', function (e) {
      //   e.preventDefault();
      ...
      // Reload Card
      // $('a[data-action="reload"]').off('click').on('click', function () {
      ...
      // Close Card
      // $('a[data-action="close"]').off('click').on('click', function () {
      ...
      // $('.card .heading-elements a[data-action="collapse"]').off('click').on('click', function () {
      //   var $this = $(this),
      ...
      if (menuType != 'vertical-compact-menu' && menuType != 'horizontal-menu' && compactMenu === false) {
        if ($body.data('menu') == 'vertical-menu-modern') {
          if (sessionStorage.getItem("menuLocked") === "true") {
      ...
      // $(".heading-elements-toggle").off('click').on("click", function () {
