...
export class FileUploadDirective implements AfterViewInit, OnChanges {
  ...
  RegisterFileUPload() {
    var option = {
      ...
      add:  (e: any, data: { originalFiles: any[]; files: string | any[]; submit: () => void; }) =>{
        var uploadErrors: string[] = [];
        var acceptFileTypes: RegExp;
        if (this.fileType == 0) {
      ...
      success:  (result: any, textStatus: any, jqXHR: any) => {
        if (this.typeUpload != 'chat_file') {
          this.blockUI.stop();
        }
        if (result) {
