...
export class EditPatientProfileComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  save() {
    ...
    if (entity?.PatientUser?.DOB) {
      entity.PatientUser.DOB = this.datePipe.transform(entity.PatientUser.DOB, 'MM/dd/yyyy');
    }
    this.patientProfileService.Save(entity).subscribe(r => {
      if (r) {
      ...
      this.dialog.showToastrError('Patient Profile', message);
    })
  }
  ...
  resetInsuranceInfo(event) {
    ...
    $(this.select.valueAccessor.element).select2().val(undefined);
  }
}
